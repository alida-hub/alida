...
public abstract class ALDBatchOutputManager {
	 ...
	 */
	private static String providedOutputClassesMethodName = "providedClasses";	
	...
	protected static HashMap<Class<?>, String> initMapTable( 
		...
		Index<ALDBatchOutputProvider,ALDBatchOutputSummarizer> indexItems = 
			SezPozAdapter.load(ALDBatchOutputProvider.class,
													ALDBatchOutputSummarizer.class);
		for (final 
					IndexItem<ALDBatchOutputProvider,ALDBatchOutputSummarizer> item : 
																																indexItems ) {
			...
			try {
				if (!interfaceRequired.isAssignableFrom( Class.forName( className))) {
					...
					Method method = 
							Class.forName(className).getDeclaredMethod(
																		providedOutputClassesMethodName, params);

					ALDBatchOutputSummarizer provider = 
						(ALDBatchOutputSummarizer)(Class.forName( className).newInstance());
					Object paramsObj[] = {};
				...
				System.err.println( 
					"ALDBatchOutputManager::initMapTable cannot create an instance for " 
																																+ className);
			}
		}
		return mapTable;
	 ...
	 * <code>interfaceRequired</code> (which by itself has to implement 
	 * {@link ALDBatchOutputSummarizer}).
	 *
	 * @param		cl								Class to get a provider for.
	 * @param		interfaceRequired	Interface provider should implement.
	 * @return	Provider instance or null if no provider found.
	 * @throws ALDBatchIOManagerException 
	 */
	public ALDBatchOutputSummarizer getProvider( Class<?> cl, 
																								Class<?> interfaceRequired) 
			throws ALDBatchIOManagerException {
...
//		}
		...
		try {
			...
			if ( ! interfaceRequired.isAssignableFrom( providerClass ) ) {
				System.err.println("ALDBatchOutputManager::getProvider " +
					"ALDBatchOutputManager: Provider class does not implement " + 
													interfaceRequired.getName() + " , invalid class!");
				return null;
			}

			return (ALDBatchOutputSummarizer)providerObj;
			...
			throw new ALDBatchIOManagerException(
					ALDBatchIOManagerExceptionType.PROVIDER_INSTANTIATION_ERROR, 
					"ALDBatchOutputManager: Provider instantiation failed!");
		} catch (IllegalAccessException e) {
			throw new ALDBatchIOManagerException(
					ALDBatchIOManagerExceptionType.UNSPECIFIED_ERROR, 
					"ALDBatchOutputManager: Illegal access noticed!");
		}
	}
}
